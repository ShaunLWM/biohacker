    ...
    <main className="page-wrap px-4 pb-8 pt-14">
      <section className="island-shell rise-in relative overflow-hidden rounded-[2rem] px-6 py-10 sm:px-10 sm:py-14">
        ...
        <h1 className="display-title mb-5 max-w-3xl text-4xl leading-[1.02] font-bold tracking-tight text-[var(--sea-ink)] sm:text-6xl">
          Launch a Firecracker VM, get SSH access, and let TTL cleanup handle
          the rest.
        </h1>
        <p className="mb-8 max-w-2xl text-base text-[var(--sea-ink-soft)] sm:text-lg">
          V1 keeps the product tight: one bare-metal host, one Node daemon, one
          create button, one shutdown action, and a configurable expiration
          timer per instance.
        </p>
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {createVmMutation.isPending ? 'Creating VM...' : 'Create VM'}
          </button>
          <a
            href="/about"
              ...
              {vms.map((vm) => (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="island-kicker mb-2">VM {vm.id.slice(0, 8)}</p>
